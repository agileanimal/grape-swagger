...
describe "a simple mounted api" do
  ...
  it "retrieves swagger-documentation on /swagger_doc" do
    get '/swagger_doc'
    last_response.body.should == "{:apiVersion=>\"0.1\", :swaggerVersion=>\"1.1\", :basePath=>\"http://example.org\", :operations=>[], :apis=>[{:path=>\"/swagger_doc/simple.{format}\"}, {:path=>\"/swagger_doc/simple_with_headers.{format}\"}, {:path=>\"/swagger_doc/simple_with_params.{format}\"}, {:path=>\"/swagger_doc/swagger_doc.{format}\"}]}"
  end
  ...
  it "retrieves the documentation for mounted-api that includes params with desciptions" do
    get '/swagger_doc/simple_with_params'
    last_response.body.should == "{:apiVersion=>\"0.1\", :swaggerVersion=>\"1.1\", :basePath=>\"http://example.org\", :resourcePath=>\"\", :apis=>[{:path=>\"/simple_with_params.{format}\", :operations=>[{:notes=>nil, :summary=>\"this posts something with params\", :nickname=>\"POST-simple_with_params---format-\", :httpMethod=>\"POST\", :parameters=>[{:paramType=>\"body\", :name=>\"id\", :description=>\"Need an id.\", :dataType=>\"Integer\", :required=>true}, {:paramType=>\"body\", :name=>\"name\", :description=>\"Name is not required.\", :dataType=>\"String\", :required=>false}]}]}]}"
  end
end
